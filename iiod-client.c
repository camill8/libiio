...
{
	if (client->reader)
		iiod_reader_destroy(client->reader);
	if (client->responder)
				       ...
				       const struct iio_device **trigger)
{
	...
	ret = iiod_reader_exec_simple_command(client->reader, &cmd);
	if (ret == -ENODEV)
		*trigger = NULL;
	else if (ret >= 0)
				       ...
				       const struct iio_device *trigger)
{
	...
	return iiod_reader_exec_simple_command(client->reader, &cmd);
}
					 ...
					 size_t len, enum iio_attr_type type)
{
	...
	return iiod_reader_exec_command(client->reader, &cmd, NULL, &iiod_buf);
}
...
static int iiod_client_enable_binary(struct iiod_client *client)
{
	...
	client->reader = iiod_responder_create_reader(client->responder);
	if (!client->reader) {
		prm_err(client->params, "Unable to create reader\n");
		iiod_responder_destroy(client->responder);
		client->responder = NULL;
		return -ENOMEM;
	}
			      ...
			      size_t samples_count, bool cyclic)
{
	...
	if (client->responder) {
		io->reader = iiod_responder_create_reader(client->responder);
		if (!io->reader)
			goto err_free_io;
				...
				size_t samples_count, bool cyclic)
{
	...
	ret = iiod_reader_exec_command(io->reader, &cmd, &buf, NULL);
	if (ret < 0)
		return ret;

	if (!device_is_tx(dev))
		iiod_reader_get_response_async(io->reader, io->buffers[1], 2);
...
int iiod_client_close_unlocked(struct iiod_client_io *io)
{
	...
	if (io->client->responder) {
		...
		ret = iiod_reader_exec_simple_command(io->client->reader, &cmd);
	} else {
...
static int iiod_client_io_upload_buffer(struct iiod_client_io *io, size_t bytes)
{
	...
	return iiod_reader_send_command(io->reader, &cmd,
...
static int iiod_client_io_swap_buffers(struct iiod_client_io *io, size_t bytes)
{
	...
	return iiod_reader_get_and_request_response(io->reader,
						    io->buffers[next], 2);
}
